...
static void ret_check() {
    ...
    void *my_data;
    dr_safe_read((void *) xsp_val, sizeof(void *), &my_data, NULL);
...
static void instrument_ret_checker(void *drcontext, instrlist_t *ilist,
        instr_t *where, instr_t *app_instrs, void *user_data, void *spill_data) {
...
static void register_ret_checker() {
    ...
    ib_hook_cache_to_instr_ex(ret_checker, OP_ret, &hndle_data,
            IB_OPT_ADDR_OPND_INFO | IB_OPT_PC_INFO | IB_OPT_FULL_OPND_INFO,
            IB_INSTRUM_BEFORE, ub_is_ret_instr, IB_GUARD_HOOK);
}
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char *argv[]) {
    ...
    taint_bunny = tb_simd_init(id, IB_OPT_BARE, &te_options, NULL, &taint_policy);
