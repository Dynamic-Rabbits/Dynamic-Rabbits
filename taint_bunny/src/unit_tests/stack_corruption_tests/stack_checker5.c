    ...
    if (input_data->type == UB_FD_FILE) {
        ...
        if (strstr(input_data->source_info.file_info.base_name, ".so") == NULL) {
            ...
            for (int i = 0; i < input_buf->size; i++) {
                ...
                tb_taint_mem_block(taint_bunny, (uintptr_t) input_buf->buf + i,
                        1, taint_policy.get_default_label());
            }
...
static void ret_check() {
    ...
    uintptr_t xsp_val = ib_get_comp_opnd(dr_get_current_drcontext(),
            &ret_addr_slot);
    void **xsp_conc_val = (void **) ib_get_comp_conc(dr_get_current_drcontext(),
            &ret_addr_slot);
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char *argv[]) {
    ...
    taint_bunny = tb_cc_init(id, IB_OPT_BARE, &te_options,
    NULL, &taint_policy);
