/*
 * tb_max6_instruction_handler_driver.c
 *
 *      Author: john
 */

#include "tb_raw_instruction_handler_driver.h"
...
tb_te_instr_hndlrs_t *tb_ih_raw_init_instr_handlers(tb_context_t *tb_ctx,
    ...
    if (tb_ctx->is_inline) {
        tb_ih_raw_in_init_instr_hndlrs(instr_hndlrs, pre_info, post_info,
                tb_ctx);
    } else {
        tb_ih_raw_cache_init_instr_hndlrs(instr_hndlrs, pre_info, post_info,
                tb_ctx);
    }
