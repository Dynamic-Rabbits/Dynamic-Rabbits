/*
 * tb_ta_raw_byt_src_srcdst_srcdst.c
 *
 *      Author: john
 */
...
static void tb_raw_handle_split_src_srcdst_srcdst_dreg_dreg_sreg_sreg_sreg(
    ...
    tb_raw_meet_from_2_src(instr_hndlr_info->tb_ctx, 0, size, drcontext, ilist, where,
            tb_raw_get_app_pc(app_instr), meet_reg2, srcdst_reg_id1, srcdst_reg_id2,
            spill_info->scaler_regs[3]);

    tb_raw_apply_to_2_reg_from_2_src(instr_hndlr_info, drcontext, ilist, where,
            tb_raw_get_app_pc(app_instr), srcdst_reg_id2, 0, size, srcdst_reg_id1,
    ...
    tb_raw_check_post_propagate(instr_hndlr_info, drcontext, ilist,
            where, spill_info->scaler_regs[4], spill_info->scaler_regs[3]);
}

static void tb_raw_handle_split_src_srcdst_srcdst_dreg_dreg_smem_sreg_sreg(
        tb_raw_instr_hndlr_info_t *instr_hndlr_info, size_t size,
        void *drcontext, instrlist_t *ilist, instr_t *where, instr_t *app_instr,
    ...
    tb_raw_meet_from_1_src(instr_hndlr_info->tb_ctx, 0, size, drcontext, ilist, where,
            tb_raw_get_app_pc(app_instr), spill_info->scaler_regs[4],
            src_mem_id1, spill_info->scaler_regs[3]);

    reg_id_t meet_reg2 = src_mem_id1;
...
void tb_raw_byt_handle_src_srcdst_srcdst_dreg_1_dreg_1_smem_1_sreg_1_sreg_1(
    ...
    tb_raw_handle_split_src_srcdst_srcdst_dreg_dreg_smem_sreg_sreg(
            instr_hndlr_info, 1, drcontext, ilist, where, app_instr,
...
void tb_raw_byt_handle_src_srcdst_srcdst_dreg_2_dreg_2_smem_2_sreg_2_sreg_2(
    ...
    tb_raw_handle_split_src_srcdst_srcdst_dreg_dreg_smem_sreg_sreg(
            instr_hndlr_info, 2, drcontext, ilist, where, app_instr,
            srcdst_reg_id1, srcdst_reg_id2, src_mem_id1, spill_info);
}
...
void tb_raw_byt_handle_src_srcdst_srcdst_dreg_1_dreg_1_sreg_1_sreg_1_sreg_1(
    ...
    tb_raw_handle_split_src_srcdst_srcdst_dreg_dreg_sreg_sreg_sreg(
            instr_hndlr_info, 1, drcontext, ilist, where, app_instr,
