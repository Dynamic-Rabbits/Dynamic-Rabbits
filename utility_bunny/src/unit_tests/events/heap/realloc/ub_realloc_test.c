...
void got_realloc_data(ub_ev_realloc_data_t *data) {
	...
	if (data->passed_addr == allocated){

		DR_ASSERT_MSG(data->size == 2222, "ub_relloc_test1 - 1 failed...");
		DR_ASSERT_MSG(data->addr != NULL, "ub_relloc_test1 - 2 failed...");

		reallocated = data->addr;
...
static void event_exit() {
	...
	DR_ASSERT_MSG(realloc_count == 8, "ub_delloc_test1 - 3 failed...");
