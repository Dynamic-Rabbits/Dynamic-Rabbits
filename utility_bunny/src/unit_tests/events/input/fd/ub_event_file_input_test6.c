/**
 *  @file ub_event_file_input_test6.c
 *  @brief 
...
static void got_read(ub_fd_input_data_t *input_data) {
	...
	DR_ASSERT_MSG(input_data->buf_list != NULL,
			"ub_file_input_option_test6 - 0 failed...");

	DR_ASSERT_MSG(ub_list_get_size(input_data->buf_list) == 1,
			"ub_file_input_option_test6 - 1 failed...");
	...
	if (input_data->offset == 10) {
		...
		DR_ASSERT_MSG(memcmp(input_buf->buf, "e!", input_buf->size) == 0,
				"ub_env_var_input_option_test6 - 2 failed...");

		DR_ASSERT_MSG(input_buf->size == 2,
				"ub_env_var_input_option_test6 - 3 failed...");
	} else if (input_buf->size == 12) {

		found_total_read = true;

		DR_ASSERT_MSG(found_offset == true,
				"ub_env_var_input_option_test6 - 4 failed...");

		DR_ASSERT_MSG(
				memcmp(input_buf->buf, "Hello there!", input_buf->size) == 0,
				"ub_env_var_input_option_test6 - 5 failed...");
...
static void event_exit() {
	...
	DR_ASSERT_MSG(count >= 1,
			"ub_event_file_input_test6 -6 failed...");

	DR_ASSERT_MSG(found_total_read == true,
			"ub_event_file_input_test6 -7 failed...");
