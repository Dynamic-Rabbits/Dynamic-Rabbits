...
static void got_read(ub_fd_input_data_t *input_data) {
	...
	DR_ASSERT_MSG(input_data->buf_list != NULL,
			"ub_file_input_option_test4 - 0 failed...");

	DR_ASSERT_MSG(ub_list_get_size(input_data->buf_list) == 1,
			"ub_file_input_option_test4 - 1 failed...");
	...
	DR_ASSERT_MSG(memcmp(input_buf->buf, "Hello there!", input_buf->size) == 0,
			"ub_file_input_option_test4 - 2 failed...");

	DR_ASSERT_MSG(input_buf->size == 12,
			"ub_file_input_option_test4 - 3 failed...");

	char *basename = "ub_file_input_test1.txt";
	DR_ASSERT_MSG(
			memcmp(input_data->source_info.file_info.base_name, basename,
					strlen(basename)) == 0,
			"ub_file_input_option_test4 - 4 failed...");
}

static void event_exit() {

	DR_ASSERT_MSG(count == 1,
			"ub_file_input_option_test4 - 5 failed...");
