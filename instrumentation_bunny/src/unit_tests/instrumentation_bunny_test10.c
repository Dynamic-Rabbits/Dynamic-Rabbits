...
void ib_clean_call_def(int value) {

    clean_called_2 = true;
    DR_ASSERT_MSG(value == 2, "ub_code_cache_test - test4 failed...");

    DR_ASSERT_MSG(ib_get_opcode(dr_get_current_drcontext()) != OP_add,
            "ub_code_cache_test - test4 failed...");
}

void ib_clean_call_add(int value) {

    clean_called_3 = true;
    DR_ASSERT_MSG(value == 5, "ub_code_cache_test - test3 failed...");

    DR_ASSERT_MSG(ib_get_opcode(dr_get_current_drcontext()) == OP_add,
            "ub_code_cache_test - test5 failed...");
}
...
void ib_code_cache_insert_hndle_func_test2(void *drcontext, instrlist_t *bb,
    ...
    count2++;
    dr_insert_clean_call(drcontext, bb, where, ib_clean_call_def, false, 1,
    OPND_CREATE_INTPTR(2));
}
void ib_code_cache_insert_hndle_func_test3(void *drcontext, instrlist_t *bb,
        instr_t *where, instr_t *app, void *user_data, void *spilling_data) {

    DR_ASSERT_MSG(user_data == (void * ) 10,
            "ub_code_cache_test - test2 failed...");

    count3++;
    dr_insert_clean_call(drcontext, bb, where, ib_clean_call_add, false, 1,
    OPND_CREATE_INTPTR(5));
}
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char *argv[]) {
    ...
    ib_hook_cache_to_default(instrum_tool, &cc_data2,
    IB_OPT_OPCODE_INFO | IB_OPT_OPND_COUNT_INFO | IB_OPT_PC_INFO);

    ib_insert_hndle_data_t cc_data3;
    cc_data3.user_data = (void *) 10;
    cc_data3.user_instrum_func = ib_code_cache_insert_hndle_func_test3;
    ...
    ib_hook_cache_to_instr(instrum_tool, OP_add, &cc_data3,
