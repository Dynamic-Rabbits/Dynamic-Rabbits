...
void ib_code_cache_insert_hndle_func_test(void *drcontext, instrlist_t *bb,
    ...
    count++;
    dr_insert_clean_call(drcontext, bb, instr, ib_clean_call, false, 1,
    OPND_CREATE_INTPTR(4));
}
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char *argv[]) {
    ...
    inline_data.user_instrum_func = ib_code_cache_insert_hndle_func_test2;
    inline_data.spill_reg_func = NULL;
    inline_data.restore_reg_func = NULL;
    inline_data.spilling_user_data = NULL;

    ib_hook_to_instr(instrum_tool, OP_add, &inline_data,
    IB_OPT_OPCODE_INFO | IB_OPT_OPND_COUNT_INFO);

    inline_data.user_data = (void *) 10;
    inline_data.user_instrum_func = ib_code_cache_insert_hndle_func_test;

    ib_hook_to_instr(instrum_tool, OP_mov_st, &inline_data,
