...
void ib_clean_call(int value) {
    ...
    DR_ASSERT_MSG(opcode == OP_mov_imm, "ub_code_cache_test - test3 failed...");
    ...
    DR_ASSERT_MSG(type == UB_IMMED_TYPE,
            "ub_code_cache_test - test3 failed...");

    int *conc_val = (int *) ib_get_comp_conc(dr_get_current_drcontext(), &slot_acces);
    ...
    DR_ASSERT_MSG(*conc_val == opnd_get_immed_int(instr_get_src(&instr, 0)),
            "ub_code_cache_test - test3 failed...");
...
bool check_hook_guard_func_test(instr_t *instr) {
    ...
    DR_ASSERT_MSG(instr_get_opcode(instr) == OP_mov_imm,
            "ub_code_cache_test - test3 failed...");

    opnd_t src = instr_get_src(instr, 0);

    return (opnd_is_immed_int(src));
}
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char *argv[]) {
    ...
    ib_hook_cache_to_instr_ex(instrum_tool, OP_mov_imm, &cc_data,
            IB_OPT_ADDR_OPND_INFO | IB_OPT_REG_OPND_INFO | IB_OPT_IMMED_OPND_INFO | IB_OPT_OPCODE_INFO | IB_OPT_OPND_COUNT_INFO | IB_OPT_PC_INFO
