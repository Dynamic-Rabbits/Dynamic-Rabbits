...
void ib_clean_call(int value) {
    ...
    if (mod && mod->names.module_name != NULL) {
        ...
        DR_ASSERT_MSG(
                strcmp(mod->names.module_name, "libib_mod_slicer_lib.so") != 0,
                "instrument_bunny_test - test1 failed...");
    }
...
DR_EXPORT void dr_client_main(client_id_t id, int argc, const char *argv[]) {
    ...
    file_t mod_file = ub_fr_open_file_with_check("./mod.txt", DR_FILE_READ);
    sb_register_module_slicer(slicer, mod_file);
    dr_close_file(mod_file);
